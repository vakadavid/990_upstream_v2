#!/bin/bash
# ExtremeKRNL build script with SUSFS auto-version and submodule check
...
if [ -f ".gitmodules" ]; then
    if git submodule status --recursive | grep -q '^[-]'; then
        ...
        git submodule update --init --recursive || {
            echo "Failed to initialize submodules. Please check network or URLs."
            exit 1
        }
    ...
    echo "No submodules found — skipping check."
fi
echo "-----------------------------------------------"
